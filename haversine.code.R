...
colnames(py)<-c("pid","hid","age","sex","hh_serial","pernum","workid","x1","y1","x2","y2","distance")
head(py)
...
hav(ly[ly$id==py$hid[1],]$x,ly[ly$id==py$hid[1],]$y,ly[ly$id==py$workid[1],]$x,ly[ly$id==py$workid[1],]$y)
#matching the house id with the coordinates of the house in locations, taking out the x and y coordinates

##*****************************

for (i in 1:100) {
  py$dist.b.h.w[i]<-hav(ly[ly$id==py$hid[i],]$x,ly[ly$id==py$hid[i],]$y,ly[ly$id==py$workid[i],]$x,ly[ly$id==py$workid[i],]$y)
}
...
hist(py[1:1000,8]) #like in tutorial2 
head(py, n=1000)
...
start = Sys.time()
#for (i in 1:1819497) {
#USING APPLY MUCH QUICKER TO CALCULATE
py$distance = apply(py[,8:11], 1, hav_vec)#apply: the data set of interest, all the rows, haversine!)
...
start = Sys.time()
#TEST of TIME
apply(py[1:10,8:11], 1, hav_vec)
print(Sys.time()-start)
start = Sys.time()
apply_dists = apply(py[1:1000,8:11], 1, hav_vec)
print(Sys.time()-start)
...
movement_by_type = merge(people_mat, loc_labels)#merging the shortened py and ly with "workid" , organized by distance small to large, and number of pid
